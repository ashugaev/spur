...
REVIEW FOCUS
  ...
  Config docs: flag config or interface changes that skip docs/configuration.md, docs/commands.md, or the spur SKILL.md mirrors.
  Hardcoded model versions: flag a pinned model version literal in source; a per-agent default must be a version-free alias registered in the one per-agent default map, with the pinned version living in config.

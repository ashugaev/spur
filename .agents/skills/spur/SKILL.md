...
CONFIG FOOTGUNS
  ...
  MCP servers belong in `sidecars`, not global agent config. A server in `~/.claude.json` or `~/.codex/config.toml` is inherited by every session Spur launches and loads whether the task uses it or not — through `npx`/`npm exec` that is a three-process stack, ~190 MB idle each, times the fleet. Declare browser tooling as `sidecars.playwright.autoStart: true` instead: pinned bin under plain `node`, one per opted-in session, port-reserved, torn down with the agent, swept if it orphans.
  `mcpServers` in `~/.claude/settings.json` is dead config — claude does not read it (2.1.221), and neither does Spur.

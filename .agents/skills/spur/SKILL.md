...
SAFETY
  ...
  Run each non-default-instance CLI call from a neutral cwd, never inside a repo checkout. `spur spawn` and `spur list` auto-connect the nearest `spur.yaml` upward from cwd; daemon boot merges it, handing that instance the project and its live sources, which spawn real sessions.

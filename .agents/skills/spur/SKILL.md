...
SAFETY
  ...
  Never run `spur cache --prune --yes` on a host running agents you do not own — it deletes files under `~/.npm`, `~/.cache`, and `/tmp` outside `~/.spur`. Plain `spur cache` (no flags, or `--prune` alone) is a dry run and the only safe form elsewhere.

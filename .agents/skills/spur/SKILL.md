...
- Daemon HTTP API, the same surface the web UI uses: `GET /info`, `GET /sessions`, `POST /sessions`, `POST /sessions/:id/{send,pause,complete,kill,respawn,restore,reopen,handoff,wake,self-destruct}`. `POST /sessions` body: `{project, prompt?, steps?, agent?, model?, planMode?, restrictWrites?, branch?, overrides?, selfDestruct?}`. `restore`/`reopen` body: `{force?}`; absent or non-object body reads as `{}`.
...
- `restore`/`reopen` refuse to launch over a live agent process still carrying that session id: a foreign process outside the pane, or the pane's own process surviving the SIGHUP, SIGTERM, SIGKILL escalation. `force: true` bypasses the foreign-process refusal only; a SIGKILL survivor refuses either way. In `spur list`, a first `r` surfaces the refusal and a second `r` on the same selection retries with force; `spur reopen <sessionId> --force` is the CLI equivalent (no override on the send/relaunch path — it has no user-facing surface to carry one). The guard reads `/proc/<pid>/environ`, so off Linux it is skipped, not failed. `spur doctor`'s `agent-process-ownership` check reports the same unowned processes at `warn`.

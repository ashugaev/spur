...
CONFIG FOOTGUNS
  ...
  Auto update: `autoUpdate` lets the daemon's reaper tick run a switch to a newer published version by itself. An accepted `POST /deploy/switch` clears the flag; the tick's own switch does not — a self-updated host stays armed. A failed candidate retries every tick, no cap, unless the recorded `failureKind` says the install already changed the host. The kind comes from the failing branch in `v2/scripts/install-and-restart.sh`, never from the exit code, and the daemon's exit handler never overwrites a terminal helper record. Such a kind on the tick's own attempt disarms `autoUpdate`; the disarm sits in the tick, never in `finishSwitch` — a successful reinit kills the daemon before it runs. Set `autoUpdate: false` before pinning with `spur update`; pin first and the next tick moves the host off the pin. Contract `docs/configuration.md#auto-update`; routes `docs/commands.md#daemon-http-api`; implementation `v2/src/auto-update.ts`, `v2/src/update-ledger.ts`, manual disarm `v2/src/server.ts`.

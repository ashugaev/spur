...
- MCP servers belong in `sidecars`, not in global agent config. A server declared in `~/.claude.json` or `~/.codex/config.toml` is inherited by every session Spur launches and loads whether the task uses it or not — through `npx`/`npm exec` that is a three-process stack, ~190 MB idle each, multiplied by the fleet. Declare browser tooling as `sidecars.playwright.autoStart: true` instead: the pinned bin under plain `node`, one per opted-in session, port-reserved, torn down with the agent, and swept if it orphans. `mcpServers` in `~/.claude/settings.json` is dead config — claude does not read it (2.1.221), and neither does Spur.
- `projects.<id>.mcp.exclude: [<name>]` suppresses an inherited server for a project. claude gets `--mcp-config <generated> --strict-mcp-config` with the name dropped; codex gets its `[mcp_servers.<name>]` table stripped; cursor has no suppression path. A sidecar binding of the same name wins over the exclusion.

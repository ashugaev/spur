...
CONFIG FOOTGUNS
  ...
  Auto update: `autoUpdate` (instance, default `false`) — the daemon self-updates through the same executor a human `Switch` press uses once the npm registry publishes a version strictly newer than the running one. No new timer: folds into the existing 5-minute reaper tick, which re-reads the flag from disk every tick (never from in-memory config), so a hand edit takes effect without a restart. `~15` min worst-case detection latency (one tick plus the registry cache's 10-min TTL). Only writers: any accepted `POST /deploy/switch` (writes `autoUpdate: false`) and `POST /deploy/auto-update`. A candidate with a terminal (`succeeded`/`failed`) status record is never retried. Escape hatch: set `autoUpdate: false` first, then `spur update <version>`/`spur update --force <version>` — reversed order re-arms the daemon. Contract `docs/configuration.md#auto-update`; implementation `v2/src/auto-update.ts`.

...
- Hidden CLI commands, absent from `--help`: `daemon start|stop|restart`, `slots`, `sidecar start|stop` (per-session, `--session <id>`), `sidecar sweep [--reap] [--json]` (daemon-wide, no session scope; reports leaked sidecar process trees, `--reap` kills them), `self-destruct`, `branch`, `subscribe`, `reinit`, `update-monitor`.

...
- `question` entries render as a full-opacity attention-bordered/tinted block (`role="alert"`) with header + prompt to make the item the user must act on visually prominent; when `options` are present, claude single-select questions render clickable option buttons (`aria-label="Answer options"`) that POST an answer and show pending/answered/error status text, while cursor/codex questions and claude multi-select questions render the same options as a static numbered list with a hint that replies go through the message box below; when options are absent, only header/prompt show

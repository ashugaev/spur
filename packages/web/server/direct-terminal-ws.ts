...
export interface DirectTerminalWss {
  wss: WebSocketServer;
  activeSessions: Map<string, TerminalSession>;
  shutdown: () => void;
}
...
/**
 * Shared WebSocket connection handler for DirectTerminal.
 * Used by both standalone server and embedded (noServer) modes.
 */
...
/**
 * Create a DirectTerminal WebSocketServer in noServer mode.
 * The caller is responsible for handling HTTP upgrade events and
 * forwarding them via wss.handleUpgrade(). Used by the unified
 * custom server to serve WebSocket on the same port as Next.js.
 */
export function createDirectTerminalWss(tmuxPath?: string): DirectTerminalWss {
  ...
  wss.on("connection", (ws, req) => {
    handleConnection(ws, req, TMUX, activeSessions);
  });
...
/**
 * Create the direct terminal WebSocket server (standalone mode with own HTTP server).

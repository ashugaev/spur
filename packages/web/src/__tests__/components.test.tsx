...
describe("Dashboard", () => {
  ...
  it("does not steal browser find from other editable targets or open dashboard modals", async () => {
    ...
    const searchInput = screen.getByPlaceholderText("Filter sessions...");
    const projectFilter = screen.getByRole("combobox", { name: "Project filter" });

    projectFilter.focus();
    expect(fireEvent.keyDown(projectFilter, { key: "f", ctrlKey: true, cancelable: true })).toBe(
      true,
    );
    expect(projectFilter).toHaveFocus();

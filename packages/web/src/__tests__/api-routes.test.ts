...
describe("Spur web API routes", () => {
  ...
  it("POST /api/sessions/:id/sidecars/:name/start proxies to daemon", async () => {
    mockedSpurRequestJson.mockResolvedValue(sessionFixture({ sidecars: [{ name: "dev", alive: true }] }));
  ...
  it("POST /api/sessions/:id/sidecars/:name/stop proxies to daemon", async () => {
    mockedSpurRequestJson.mockResolvedValue(
      sessionFixture({ sidecars: [{ name: "dev", alive: false }] }),
    );
  ...
  it("POST /api/sessions/:id/sidecars/:name/start returns 502 on daemon error", async () => {
    mockedSpurRequestJson.mockRejectedValue(new Error("Cannot start sidecar"));

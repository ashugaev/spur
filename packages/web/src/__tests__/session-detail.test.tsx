...
describe("SessionDetail voice input", () => {
  ...
  it("starts an offline sidecar from the detail sidebar", async () => {
    vi.spyOn(global, "fetch").mockImplementation(async (input, init) => {
      const url = typeof input === "string" ? input : input.url;
      if (url === "/api/sessions/api-a1") {
        return new Response(
    ...
    const startButton = await screen.findByRole("button", { name: "Start sidecar dev" });
  ...
  it("stops a live sidecar from the detail sidebar", async () => {
    const fetchMock = vi.spyOn(global, "fetch").mockImplementation(async (input, init) => {
      const url = typeof input === "string" ? input : input.url;
      if (url === "/api/sessions/api-a1") {
        return new Response(
    ...
    const stopButton = await screen.findByRole("button", { name: "Stop sidecar dev" });

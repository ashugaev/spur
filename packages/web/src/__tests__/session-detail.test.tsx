...
describe("SessionDetail voice input", () => {
  ...
  it("renders active sidecar log issue details and sends a fix request", async () => {
    ...
    vi.spyOn(global, "fetch").mockImplementation(async (input, init) => {
      ...
      if (url === "/api/sessions/api-a1/send" && init?.method === "POST") {
        ...
        if (typeof body === "string") {
          try {
            sentBody = JSON.parse(body) as unknown;
          } catch {
            sentBody = null;
          }
        }
    ...
    expect(await screen.findByText("active issue")).toBeInTheDocument();
    expect(screen.getByText("ui-watch/typescript")).toBeInTheDocument();
    expect(
    ...
    if (resolveSend === null) {
      throw new Error("Expected sidecar fix send request");
    }
    resolveSend(new Response(JSON.stringify(sessionFixture()), { status: 200 }));
  ...
  it("renders sidecar fix errors inline", async () => {
    vi.spyOn(global, "fetch").mockImplementation(async (input, init) => {
      const url = typeof input === "string" ? input : input.url;
      if (url === "/api/sessions/api-a1") {
        return new Response(
      ...
      if (url === "/api/sessions/api-a1/send" && init?.method === "POST") {
        return new Response(JSON.stringify({ error: "Fix unavailable" }), { status: 500 });
      }
      throw new Error(`Unexpected fetch: ${url}`);
    });
    ...
    expect(await screen.findByText("Fix unavailable")).toBeInTheDocument();

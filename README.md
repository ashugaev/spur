...
Use [spur.yaml.example](v2/spur.yaml.example) as the copyable baseline. Add `symlinks`, `sources`,
...
- `--model <id>` applies to the resolved agent (from `--agent`, else the project/instance default agent) on fresh launch. Without `--model` and no project `defaultModels` entry, Spur applies its built-in default for the agent: claude → `opus`, cursor → `auto`; codex falls back to its own default. Model ids come from claude aliases (opus/sonnet/haiku/fable), codex `models_cache.json` under `CODEX_HOME`, or `agent models` for cursor.
...
projects:
  backend-api:
    ...
    defaultModels: # optional; per-agent default model, applied when that agent is chosen without an explicit model (overrides Spur's built-in default: claude opus, cursor auto)
      codex: gpt-5.5
      cursor: composer-2.5
    branchNaming:
...
Today `spur init` installs the `spur-daemon`/`spur-web` systemd user units. Flags: `--no-start`; `--expose-web` (public `0.0.0.0` bind, default `127.0.0.1`); `--web-port <port>` (default `4311`); `--tailscale`/`--no-tailscale` (default on — widens `spur-web.service` `WEB_HOST` to `127.0.0.1,<tailnet-ip>` once the tailnet is up; loopback stays bound; never `0.0.0.0`). `--expose-web` is the explicit public override and supersedes Tailscale. `WEB_HOST` takes a comma-separated host list (`packages/web/server/web-hosts.ts`); `spur-web`'s production server binds one listener per host. Full walkthrough: [install-from-npm.md](docs/install-from-npm.md).

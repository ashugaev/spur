...
- Claude runtime only. Needs `DesignSync` plus bundled `/design-login` and `/design-sync`. If `DesignSync` is unavailable, run `/design-login` once; if scopes still fail, stop and report.
...
1. Read task-memory / spec. Identify only the components the task adds or changes.

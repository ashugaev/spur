...
SAFETY
  ...
  Never run `spur cache --prune --yes` on a host running agents you do not own — it deletes files under `~/.npm` and `~/.cache/ms-playwright(-mcp)` outside `~/.spur`. Prunable classes: `vendor-cache`, `npx-package` (unless a pin source), `browser-revision` (unless pinned or pin sources unresolved). All other classes (`generic`, `tmp-entry`, `browser-profile`, `browser-registry`) are report-only and never deleted. Requires a resolved instance config; aborts non-zero if config is absent or invalid. Plain `spur cache` (no flags, or `--prune` alone) is a dry run and the only safe form elsewhere.

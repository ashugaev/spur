...
SAFETY
  ...
  `restore`/`reopen` refuse to relaunch over a live agent process still carrying that session id (a foreign process, or the pane's own process surviving the kill escalation) so a session never ends up with two live agent processes. `--force` (CLI) or a second `r` on the same selection in `spur list` bypasses only the foreign-process refusal, never a confirmed survivor. `spur doctor`'s `agent-process-ownership` check reports unowned agent processes at `warn`; detail in `docs/commands.md`.

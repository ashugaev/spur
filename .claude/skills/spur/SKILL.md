...
- Daemon HTTP API, the same surface the web UI uses: `GET /info`, `GET /sessions`, `POST /sessions`, `POST /sessions/:id/{send,pause,complete,kill,respawn,restore,handoff,wake,self-destruct}`. `POST /sessions` body: `{project, prompt?, steps?, agent?, model?, mode?, planMode?, restrictWrites?, branch?, overrides?, selfDestruct?, subscriptions?}`.
...
- `spur spawn <project> [prompt...]` is positional. Empty prompt opens a blank session and skips both default `spawn.steps` and initial message injection — unless a mode resolved, in which case the mode instruction alone becomes the initial message.
...
- Mode: `projects.<id>.modes.<name>` is `{skill, default?}`; name matches `^[a-zA-Z0-9_-]+$`, at most one `default: true` per project, both enforced at config load. Resolved once at spawn — explicit request (`--mode`, `POST /sessions` body, trigger `spawn.mode`) beats the project default beats no suffix; an unknown requested name fails the spawn and lists the configured names. Respawn, handoff, and restore carry the persisted mode forward and never re-resolve, so an unmoded session stays unmoded; a mode renamed or removed since spawn degrades to no-mode with a logged warning instead of blocking. Prompt-level and advisory. The web spawn route does not forward `mode`, so UI spawns get the project default.

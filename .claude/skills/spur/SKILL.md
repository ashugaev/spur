...
CONFIG FOOTGUNS
  ...
  Stale mode: `staleAfterMinutes` (instance, default `60`, `0` disables; `projects.<id>.staleAfterMinutes` overrides) parks a `running`/`waiting` session idle past the threshold — pane killed, sidecars torn down, `status: "stopped"`, `stopReason: "stale_timeout"`, state `stale`. Any system message or manual Resume wakes it silently, replays `staleSidecars`, and delivers after the agent process is confirmed live. Contract `docs/configuration.md#stale-mode`; implementation `v2/src/session-service.ts`.

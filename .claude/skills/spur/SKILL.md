...
- Daemon HTTP API, the same surface the web UI uses: `GET /info`, `GET /sessions`, `POST /sessions`, `POST /sessions/:id/{send,pause,complete,kill,respawn,restore,reopen,handoff,wake,self-destruct}`. `POST /sessions` body: `{project, prompt?, steps?, agent?, model?, planMode?, restrictWrites?, branch?, overrides?, selfDestruct?}`. `restore`/`reopen` body: `{force?}` — bypasses the duplicate-agent-process refusal (never the pane's own SIGKILL-survivor refusal).
...
- `restore`/`reopen` refuse when a live agent process for that session id already exists — the prior pane's own process survived SIGKILL, or a foreign process elsewhere on the host carries the session id. `force: true` bypasses the foreign-process check only; a SIGKILL survivor always still refuses. `spur list`'s `r` key: first press surfaces the refusal, second press on the same selection retries with force.

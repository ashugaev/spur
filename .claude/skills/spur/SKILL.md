...
CONFIG FOOTGUNS
  ...
  Stale mode: `staleAfterMinutes` (instance, default `720` = 12h, `0` disables; `projects.<id>.staleAfterMinutes` overrides) parks a `running`/`waiting` session idle past the threshold, keyed on genuine transcript activity only (never a routine record write, never tmux attach) — pane killed, sidecars torn down, `status: "stopped"`, `stopReason: "stale_timeout"`, state `stale`. Waking one goes through the same admission gate as spawn/restore; a refused scheduled/interval/daily wake re-arms instead of dropping, a refused trigger/queued delivery retries through its own path. Any system message or manual Resume wakes it silently, replays `staleSidecars`, resends the original task prompt first if no native resume was available (fresh launch), and delivers after the agent process is confirmed live. Contract `docs/configuration.md#stale-mode`; implementation `v2/src/session-service.ts`.

...
Config registry (`config-registry.json` in `dataDir`) never takes a config path inside `worktreeDir` — auto-connect skips it, `POST /projects/connect` returns 400, same for a non-absolute path on either `connect` or `disconnect`. Boot and every connect/disconnect drop entries that are not an existing file, worktree-internal leftovers, and same-realpath duplicates (boot keeps an entry on an indeterminate stat error instead of dropping it on a guess); boot persists the survivors and logs `daemon.registry.pruned`. Completing/killing a session also unregisters its worktree config, but only when that path outlived the prune above — i.e. it was registered under a `worktreeDir` the host has since reconfigured away from; the common in-`worktreeDir` case is already covered by the prune. `spur connect|disconnect [path]` resolve a relative path against the cwd. `spur doctor` check `config-registry`: `warn`, never the exit code, flags dead, worktree-internal, or over-24 entries; reads `SPUR_CONFIG` or the default instance config, ignores `--config`.

...
- `spawn` is positional: `spur spawn <project> [prompt...]` with optional `--agent claude|codex|cursor`, `--model <id>`, `--mode <name>`, `--branch <name>`, `--plan`, `--restrict-writes`, repeatable `--step <label>`, and either `--worktree [defaultBranch]` or `--shared`. Empty prompt opens a blank session and skips default pipeline steps and initial message injection — unless a mode resolved, in which case the mode instruction alone becomes the initial message so a bare `spur spawn <project>` still routes.
...
Mode selection: `projects.<id>.modes.<name>` is `{skill: string (required, non-empty), default?: boolean}`; name matches `^[a-zA-Z0-9_-]+$`, at most one mode per project sets `default: true`, both enforced at config load. Resolved once at spawn: an explicit request (`--mode`, `POST /sessions` body, trigger `spawn.mode`) beats the project default, which beats no suffix; an unknown requested name fails the spawn and lists the configured names. The suffix names the skill the session loads. Respawn/handoff/restore reuse the persisted `mode` and never re-resolve, so a session with no mode stays without one; if the persisted mode was renamed or removed from config since spawn, the carried-forward lookup degrades to no-mode with a logged warning instead of blocking. Prompt-level and advisory only. The web spawn route does not forward `mode`, so UI spawns get the project default.

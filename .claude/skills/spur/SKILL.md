...
- Isolated daemon configs inherit allowlisted user keys (currently `voice`); server, data, tmux stay isolated. Extend `INHERIT_KEYS` in `v2/src/isolated-instance-config.ts` to propagate more.

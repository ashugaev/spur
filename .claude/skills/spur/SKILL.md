...
CONFIG FOOTGUNS
  ...
  Auto update: `autoUpdate` lets the daemon's reaper tick run a switch to a newer published version by itself. An accepted `POST /deploy/switch` clears the flag; the tick's own switch does not — a self-updated host stays armed. A failed candidate is retried every tick, no cap, unless its recorded `failureKind` says the package installed (`rolled_back`, `install_unhealthy`) or the record says `succeeded`; every suppression and every retry logs to `events.jsonl`. The kind is written where the failure happens (`v2/scripts/install-and-restart.sh`), never re-derived from the exit code, and the daemon never overwrites the helper's record. Set `autoUpdate: false` before pinning with `spur update`; pin first and the next tick moves the host off the pin. Contract `docs/configuration.md#auto-update`; routes `docs/commands.md#daemon-http-api`; implementation `v2/src/auto-update.ts`, disarm `v2/src/server.ts`.

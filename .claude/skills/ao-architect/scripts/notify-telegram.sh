...
# Usage: notify-telegram.sh "<summary>" "<questions>" "<resolved>"
...
PAYLOAD=$(python3 -c "
import json, sys
issue, session, chat_id, summary, questions, resolved = sys.argv[1:]
text = (
    f'*[ao] {issue}*\n'
    f'Session: \`{session}\`\n\n'
    f'*What I understood:*\n{summary}\n\n'
    f'*Product questions:*\n{questions}\n\n'
    f'*Resolved myself:*\n{resolved}\n\n'
    f'Reply here \u2014 the agent will receive your answer automatically.'
)
print(json.dumps({
    'chat_id': chat_id,
    'text': text,
...
" "$AO_ISSUE_ID" "$AO_SESSION" "$TELEGRAM_CHAT_ID" "$SUMMARY" "$QUESTIONS" "$RESOLVED")

...
- Single theme object (`v2/src/cli-view.ts`): brand accent `#f04c4c` (ids, tiny loading frames), brand mark `𖤓` (help headers, section headers, spinner). State dot: green `working`, yellow `waiting`/`needs_input`/`rate_limited`, accent-red `error`, gray everything else.

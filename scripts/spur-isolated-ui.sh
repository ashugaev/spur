...
PUBLIC_HOST="${SPUR_SIDECAR_PUBLIC_HOST:-}"
PUBLIC_BASE_URL="${SPUR_SIDECAR_PUBLIC_URL:-}"
...
if [[ -n "$PUBLIC_BASE_URL" ]]; then
  PUBLIC_URL="${PUBLIC_BASE_URL%/}:$UI_PORT"
...
if [[ -n "$PUBLIC_URL" ]]; then
  "$SLOT_COMMAND" --link sidecar-ui="$PUBLIC_URL" >/dev/null 2>&1 || true
fi

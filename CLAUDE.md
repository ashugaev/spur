...
ALWAYS-ON RULES
  ...
  Change to `scripts/main-deploy.sh`, `deploy/*.service`, or the source-install deploy path: run the deploy end-to-end on the itest VM before merge. The `tests/deploy` suite stubs `systemctl`, `curl`, `ss`, and `pnpm`, so it proves nothing about real systemd, real startup timing, or a real Next build. Verified means: the script runs to completion, both units come up active, the web answers, the chunk guard passes, plus one negative case — force a mid-deploy abort and confirm `spur-web` is left active. Box name, IP, and key live only in the host-local recipe `~/.spur/itest-conn.md`, never in a repo file; reset in place with `tests/itest/reset-vm.sh`; never create, delete, or stop the box.

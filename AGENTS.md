...
ALWAYS-ON RULES
  ...
  Change to `scripts/main-deploy.sh`, `deploy/*.service`, or the source-install deploy path: verify end to end on the itest VM before merge, per `.agents/skills/clean-install-test/SKILL.md` SOURCE-INSTALL DEPLOY MODE.

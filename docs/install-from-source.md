...
- Spur pins every agent session's `NPM_CONFIG_PREFIX` to `~/.local`, so `claude`/`codex` self-update (`npm install -g ...`) lands binaries under `~/.local/bin` regardless of this `~/.npm-global` layout — both daemon unit `PATH`s list `~/.local/bin` first, so the self-updated copy wins for Spur sessions.

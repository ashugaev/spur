...
- Spur pins every agent session's `NPM_CONFIG_GLOBALCONFIG` (both env casings) at `~/.spur/npmrc`, a Spur-owned file resolving `prefix=~/.local` (see [install-from-npm.md](install-from-npm.md#setup)), so `claude`/`codex` self-update lands under `~/.local/bin` regardless of this `~/.npm-global` layout — the daemon unit's `PATH` lists `~/.local/bin` first, so the self-updated copy wins for Spur sessions. Sidecars/services/the login pane strip this pin so they can source `~/.nvm/nvm.sh` without tripping nvm's own `NPM_CONFIG_PREFIX`/`~/.npmrc` incompatibility guards. Every `daemon start` writes `~/.spur/npmrc` unconditionally (this path never runs `npm-init.sh`, so nothing else would create it) — but this deploy never heals a stray `prefix=`/`globalconfig=` line in `~/.npmrc` itself (that half is `spur init`/`update`/`reinit`-only, and mutating a shared file on every restart isn't safe); `spur doctor`'s `npmrc-nvm-conflict` check flags one and gives the exact line to remove by hand.

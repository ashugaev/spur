...
`POST /shepherd/spawn` reuses the newest running or spawning Shepherd, skipping one that already holds a Telegram reply target — such a session (including one hand-bound with Telegram `/watch`) is chat-owned and spawns a fresh Shepherd instead of hijacking it. Pass `reportDisposition: true` to receive `{ disposition: "spawned" | "reused", session }`; omit it for the legacy session-only response.

...
A recurring wake (`--every` or `--daily-at`) only fires while the session's status is restorable (`running`, `stopped`, `paused`) — `send` resumes a `stopped`/`paused` session automatically, so its wake fires normally. Once the session goes `killed`, the daemon drops the schedule on its next tick and logs `session.wake.interval_cancelled` (or `session.wake.daily_cancelled` for a daily-only wake) instead of repeatedly failing to send. `errored` and `completed` sessions keep the recurring wake armed but silent instead: it stays due without firing or advancing until the session comes back through `restore` or `reopen`, at which point the first tick after that fires one immediate catch-up wake. A restorable session that cannot be recovered — its tmux pane is gone, or the pane is live but the agent process inside it has exited — while its workspace is also missing (shepherd excepted, see above) also stays armed but silent: the daemon logs `session.wake.suppressed` once on the transition into that state, not once per tick, and resumes with a single catch-up wake on the first tick after the workspace or pane is recovered.
...
`pause` keeps the worktree. `complete` and `kill` both tear down the pane and remove an owned worktree; `kill` additionally requires `--force` on a dirty or unpushed worktree. Both check for an open GitHub PR before tearing down: `--pr-action leave_open` or `--pr-action close` answers the check when one is open, and `--skip-pr-check` skips the check entirely (no `gh` calls). When the check itself fails — rate-limited, or (commonly) `gh` missing, unauthenticated, or unreachable, though any error raised while checking is reported this way — the command fails with a message naming the likely cause and the `--skip-pr-check` retry. Shared-workspace sessions keep the project path on `kill`. `restore` needs an existing workspace (shepherd excepted, see above) plus one of: status `running` with state `stopped`, status `stopped` with state `stopped`/`error`/`stale`, status `paused` with state `stopped`/`error`, or status `errored` with state `error` — so `killed` and `completed` sessions are never restorable. A [stale-parked](configuration.md#stale-mode) session is `stopped`/`stale`, so it restores like any other stopped one.
...
With ToDo enabled for its pinned session policy, a session owns one append-only daemon ledger. `add`, `complete`, `cancel`, and `hold` require a reason. `--human-action` records a human blocker; `resume` reopens held work. No delete action exists. Open or held work blocks completion, self-destruct, handoff, trigger completion, and desk completion. A host CLI or session-detail user can complete anyway with a fresh reason; item states stay unchanged and the override enters audit history. A target session cannot override itself.

`$SPUR_TODO_COMMAND` points to a session-bound `spur-todo` wrapper. It accepts the same actions without `--session` and cannot target another ledger. `$SPUR_SESSION_TOOL_DIR/spur-todo` is the explicit path when a login shell drops the tool directory from `PATH`. Disabled sessions expose neither variable nor wrapper; GET and POST return `todo_disabled` (409), and session detail omits the section.

Dashboard rows receive compact counts from the sessions response. Corrupt ledgers show `ToDo ledger corrupt`; transient read failures show `ToDo unavailable`. Disabled sessions omit ToDo dashboard state.
...
An observed `working` to `waiting` edge arms one persisted nudge for 60 seconds later when the ledger has open work or a human-held item. A mutation while waiting resets that due time when work remains eligible, or clears it when work resolves or only external holds remain. Queued messages and running pipelines suppress the nudge. Failed delivery leaves it due for the next five-second sweep; successful delivery consumes it. A later observed `working` to `waiting` edge rearms it.
...
Daemon routes: `GET /sessions/:subscriberId/subscriptions` lists records. `POST /sessions/:subscriberId/subscriptions` accepts `{ targetSessionId, states?, events?, message? }`; `states` and `events` cannot both be empty, and `events` accepts only `task_completed`. `POST /sessions/:subscriberId/subscriptions/:subscriptionId/remove` removes one record.
...
Delivery goes through the normal queued `send` path: a `stopped`/`paused` subscriber gets resumed (native conversation resume, then fresh launch fallback) to receive it. A state transition dispatch runs once; synchronous `send` failure leaves it unclaimed until a later matching transition. A failed `task_completed` dispatch stays unclaimed and retries during the next five-second reconciliation sweep. Once `send` queues either message, the subscription is claimed; a later pane-write failure retries through the message queue, not subscription dispatch. Failures log `session.subscription.delivery_failed`.

...
`spur --help` lists 28 public commands, one-line description each; `spur <command> --help` lists every flag. Hidden: `daemon start|stop|restart`, `slots`, `sidecar start|stop|ports|sweep`, `self-destruct`, `branch`, `reinit`, `update-monitor`.

## Session tools and environment

`$SPUR_SESSION_TOOL_DIR` on `PATH`: session-bound `spur`, `spur-slots`, `spur-sidecar`, `spur-self-destruct`, `spur-todo`. `branchNaming.regex` adds `spur-branch` + a push-checking `git` wrapper; hook-state agents also get `spur-agent-state`. Call each by its explicit `"$SPUR_SESSION_TOOL_DIR/<tool>"` path - login shells rebuild `PATH`, drop the tool dir. Identity: `$SPUR_SESSION`, `$SPUR_PROJECT`, `$SPUR_AGENT`, `$SPUR_SESSION_TOOL_DIR`, `$SPUR_SESSION_ARTIFACTS_DIR`, `$SPUR_REAL_HOME`. Commands: `$SPUR_SLOT_COMMAND`, `$SPUR_TODO_COMMAND`; hook-state agents add `$SPUR_AGENT_STATE_COMMAND`, `$SPUR_AGENT_STATE_FILE`.
...
Spur keeps a durable config registry in `dataDir`: any CLI command syncs its `--config` in; daemon boot reloads every registered path, rehydrates session state, resumes pipelines, restarts sources/triggers. Attached configs must agree on `server.host`, `server.port`, `dataDir`, `worktreeDir`; project ids/`sessionPrefix` stay globally unique per daemon.
...
`--model <id>` ids: claude aliases (opus/sonnet/haiku/fable), Codex `models_cache.json` under `models.codexHome`, `cursor models`, `opencode models`. `--restrict-writes` blocks file writes, allows GitHub comments/MCP calls, keeps spawn steps - Claude/Codex deny hooks, Cursor `--plan`. `--subscribe-to`/`--subscribe-state`/`--subscribe-message` arm one subscription at spawn, see [subscribe](#subscribe).
...
`wake` stores a delayed or recurring message, delivered when due. Daily wakes use daemon-local `HH:MM`; daily and `--every` both require `--until`. `--cancel` drops a recurring wake, exclusive with scheduling options. Each occurrence fires once: sync failure logs `session.wake.failed`/`daily_failed`/`interval_failed`; queued pane-write failure logs `sent`/`daily_sent`/`interval_sent`, retries via queue.

Fires only while status is restorable (`running`, `stopped`, `paused`). Once `killed`, daemon drops it, logs `session.wake.interval_cancelled`/`daily_cancelled`. `errored`/`completed` stay armed but silent until `restore`/`reopen`, then fire one catch-up wake - same for an unrecoverable-but-restorable session (`session.wake.suppressed` once, on transition).
...
While an agent is busy, `send` queues per session, flushes on return to prompt, ahead of the next auto-step. A `stopped`/`paused` session with an existing workspace: `send` tries native resume first, then a fresh launch.

`spur queue <sessionId> list|remove|flush [index]`. `list` numbers real queued messages from `1`; a pipeline's own future steps print unnumbered, never a target. `remove`/`flush` resolve that number to exact text just before acting; `remove` drops it unsent, `flush` sends it immediately, ahead of the queue. Both fail `409` only if a pane write for that message is in flight. Wire: [daemon-api.md](daemon-api.md#session-routes).

Delivery events: `session.message.sent`, `session.message.delivery_recovered` (submit ack timed out, process alive), `session.message.delivery_failed` (retried next poll, repeats suppressed after the first), `session.message.queue_removed`. Lifecycle events land in `<dataDir>/events.jsonl`, incl. a pre-spawn `host.disk.low` warning (report-only). Retention: [configuration.md](configuration.md#event-log-retention). GitHub poll-cost events, `adaptivePoll`: [configuration.md](configuration.md#events).
...
`service run` reads `SPUR_SESSION`, starts the command in a separate tmux sidecar, stores metadata under the data dir. No stop or restart - stays bound while the session is alive. Pass `--port` so `list` can surface it. Output also lands in the session event log for `service logs`/`/sessions/:id/logs`.
...
Scopes: `task` (`<dataDir>/memory/task/<workspaceId>/<key>.md`, per workspace), `project` (`<dataDir>/memory/project/<projectId>/<key>.md`, per project), `global` (`<dataDir>/memory/global/<key>.md`, one per instance). Spawn prompt tells agents to read `task`/`project` on start, write durable, high-value facts only.
...
Start dev servers/test helpers via `"$SPUR_SESSION_TOOL_DIR/spur-sidecar" --name <name>`, never bare - starts a configured sidecar from `projects.<id>.sidecars`. `autoStart` sidecars return on spawn, restore, or recover. A sidecar starting another is manual-only; nesting stops after one level.
...
Commands run through `sh -lc`, no `exec` - `/bin/sh` is `dash` on Debian/Ubuntu, nvm needs `bash -lc '. "$SPUR_REAL_HOME/.nvm/nvm.sh" && nvm use <v> && ...'`. A remapped `$HOME` still resolves via `$SPUR_REAL_HOME` (from `/etc/passwd`). Sidecars, services, the Claude OAuth pane strip the npm prefix pin (`NPM_CONFIG_PREFIX`/`npm_config_prefix`/`NPM_CONFIG_GLOBALCONFIG`/`PREFIX`) to source `~/.nvm/nvm.sh`; a sidecar's own `npm run`/`npx` re-exports it to children.

Stop/restart reap the sidecar's whole tmux pane process tree, not just the direct child. `spur sidecar sweep` reports unclaimed process trees (pid, rss, age, worktree); nothing dies without `--reap`. Daemon idle-reap: [Sidecar reaping](configuration.md#sidecar-reaping).
...
Claude launches with `--mcp-config <path> --strict-mcp-config` - only Spur-pre-merged servers survive: `~/.claude.json` user-scope servers, `~/.claude.json` `projects["<worktree path>"]`, `<worktree>/.mcp.json`. A fresh worktree has no `projects["<worktree path>"]` entry, so local-scope servers approved against the main repo path drop. A host `mcpServers.playwright` entry from any source gets replaced.

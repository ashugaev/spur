...
Each session's ToDo ledger starts empty — no code path seeds an item; the agent adds one per step, right before it, and resolves it right after. `add`/`complete`/`cancel`/`hold` require a reason; `--human-action` records a human blocker; `resume` reopens held work; no delete. A human can also `add` through the CLI or session-detail UI. An empty ledger, and open or held work, both block completion, self-destruct, handoff, trigger+desk completion, but only for an agent-initiated call. A human-initiated call — CLI or UI, with no other session attributed as caller — bypasses the gate with no reason required; `--todo-override-reason` is optional audit text recorded on the bypass (item states unchanged, enters audit history). Running `spur complete`/`self-destruct`/`handoff` from inside an agent's own pane still counts as that agent: prefix the command with `env -u SPUR_SESSION` to run it as a human.

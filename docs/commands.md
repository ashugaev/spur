...
`spur source reply <message...> [--button <label[=value]>] [--session <id>]`. Sends a Telegram message from the session to the user; session defaults to `SPUR_SESSION`. Destination: the session's latest inbound Telegram chat, else the source's `chatId`. No destination, no send — the call fails.

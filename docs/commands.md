...
`spur list --json` (and the non-TTY list row set) comes from `GET /sessions` — the projected list item; it does not carry `artifacts`, `artifactsTruncated`, `stateHistory`, `launchCommand`, `prompt`, or `originalTaskPrompt`. The interactive selector's details pane (`prompt`/`launch` rows) fetches those separately from `GET /sessions/:id` for the currently selected session, on selection change and on each 2s refresh tick — the pane shows a loading line until that fetch resolves. Wire: [daemon-api.md](daemon-api.md#session-routes).

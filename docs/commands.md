...
`pause` keeps the worktree. `complete` and `kill` both tear down the pane and remove an owned worktree; `kill` additionally requires `--force` on a dirty or unpushed worktree. Both check for an open GitHub PR before tearing down: `--pr-action leave_open` or `--pr-action close` answers the check when one is open, and `--skip-pr-check` skips the check entirely (no `gh` calls). When the check itself fails — `gh` missing, unauthenticated, rate-limited, or unreachable — the command fails with a message naming the cause and the `--skip-pr-check` retry. Shared-workspace sessions keep the project path on `kill`. `restore` needs an existing workspace (shepherd excepted, see above) plus one of: status `running` with state `stopped`, status `stopped` with state `stopped`/`error`/`stale`, status `paused` with state `stopped`/`error`, or status `errored` with state `error` — so `killed` and `completed` sessions are never restorable. A [stale-parked](configuration.md#stale-mode) session is `stopped`/`stale`, so it restores like any other stopped one.

...
`spur send <sessionId> <message>`. While an agent is busy, `send` queues per session, flushes on return to prompt, ahead of the next auto-step. A queued message is typed once the agent is `waiting` and its transcript has been quiet 2s. Direct sends (web Send now), `flush`, and queued deliveries wait at most 20s for the agent's submit ack — four 5s windows, `Enter` resent between; a live agent past that counts as delivered (`session.message.delivery_recovered`), a dead one fails. Prints `Delivered message to <id>.` when the response carries no real queued messages, `Queued message for <id> (<N> pending).` otherwise; N counts real queued messages, never a pipeline's own auto-steps. A `stopped`/`paused` session with an existing workspace: `send` tries native resume first, then a fresh launch.

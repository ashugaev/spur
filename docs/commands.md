...
- Closeout: `$SPUR_CLOSEOUT_OWNER=1` marks the assigned closeout owner for a writable worktree, including transferred handoff ownership on a reused workspace; `0` marks non-owners, including read-only and shared-workspace sessions. The Stop hook skips git and PR closeout for `0`. A missing variable preserves standalone hook enforcement outside Spur.
...
`spur disk-gc [--execute]` reclaims: stale `mcp-*` playwright profile dirs under `~/.cache/ms-playwright(-mcp)` (default target, protected by a live argv match or a live `SingletonLock`/`SingletonSocket` pid), and webpack/`.next` build caches inside a worktree where every session record sharing that `worktreePath` is `completed` or `killed` — a worktree with zero matching records is reported `orphaned_no_record` and never touched, and a `worktree: false` session's real checkout (outside `worktreeDir`) is refused (`path_outside_worktree_dir`, re-checked on the realpath immediately before delete). `--execute` re-checks every profile candidate immediately before delete, not just at plan time: it refuses a candidate that turned into a symlink or a foreign-uid dir since planning (`symlink`, `not_owned`), and it fails closed — refusing the delete (`process_list_unreadable`) — rather than treating an unreadable or empty process snapshot as "nothing has this profile open". Dry run by default; `--execute` applies the plan. Both a dry run and an executed run print every candidate path, its bytes, and its justification — per target (build caches, mcp profiles, browser revisions, npm keys) — never just a total.
...
npm cap: `disk-gc` measures `~/.npm/_cacache` itself with its own `du` call, independent of any prior `spur disk` run or `<dataDir>/disk-budget.json` — a destructive path never trusts a cached, possibly stale or absent measurement. When it exceeds `diskBudget.npmCacheMaxGb`, `--execute` runs `npm cache verify` (collects orphaned/corrupt content only), re-measures, and — only if still over cap — reads `index-v5` to rank entries oldest-first by `time` and removes exactly enough with `npm cache clean <key>` (one entry per call, never `--force`) to clear the cap, then `npm cache verify` once more. Skipped entirely while any npm/pnpm/npx/yarn process runs, and also when the process snapshot itself is unreadable or empty — an unreadable process list is never read as "nothing is running", matching `spur cache`'s fail-closed rule. An unreadable/malformed `index-v5` aborts the whole npm target (`report.npmCap.status: "index-unreadable"`) without touching it. This command never runs `npm cache clean --force` and never deletes `_cacache` itself — that whole-root wipe stays `spur cache --prune --yes`'s job (see below).
...
`spur send <sessionId> <message>`. While an agent is busy, `send` queues per session, flushes on return to prompt, ahead of the next auto-step. A queued message is typed once the agent is `waiting` and its transcript has been quiet 2s. Direct sends (web Send now), `flush`, and queued deliveries wait about 20s for the agent's submit ack — four 5s windows, `Enter` resent between; a live agent past that counts as delivered (`session.message.delivery_recovered`), a dead one fails. Prints `Delivered message to <id>.` when the response carries no real queued messages, `Queued message for <id> (<N> pending).` otherwise; N counts real queued messages, never a pipeline's own auto-steps. A `stopped`/`paused` session with an existing workspace: `send` tries native resume first, then a fresh launch.

`spur queue <sessionId> list|remove|flush [index]`. `list` numbers real queued messages from `1`; a pipeline's own future steps print unnumbered, never a target. `remove`/`flush` resolve that number to exact text just before acting; `remove` drops it unsent, `flush` sends it immediately, ahead of the queue. `flush` fails `409` while a pane write or a queued delivery attempt (settle wait, readiness check, pane write) is in flight for that session. `remove` fails `409` only when the target is the queue head and its delivery is in flight; any other position always succeeds. Wire: [daemon-api.md](daemon-api.md#session-routes).

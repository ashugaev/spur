...
- `ui.port`: optional, default `5555`. Web UI listen port. `spur-web.service` carries the same number as `Environment=PORT` and wins when both are set; `spur doctor` warns on a mismatch (`web-ui-port-drift`). Moving the port means both — `spur init --web-port <n>` for the unit, `ui.port` here. Sources that call the web UI (Telegram voice transcription, see [voice.md](voice.md#telegram-voice-notes)) post to `http://127.0.0.1:<ui.port>`, or skip transcription when `ui.port` is `0` (no known web UI). An isolated daemon (`scripts/spur-isolated-daemon.sh`) reads the outer session's own reserved `isolated-ui` sidecar port and writes it here instead of inheriting the default, so it never posts to the host's production web UI; `0` when that reservation is not (yet) known.

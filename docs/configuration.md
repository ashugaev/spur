...
- `projects.<id>.branchNaming.regex`: optional JavaScript regex. Validates explicit, trigger, and preflight branches, and the generated session-ID fallback branch a `worktree: true` spawn takes when no branch is given — a regex that excludes names like `spur-c527` rejects that spawn. Exempt: a preflight that answered `NO_PROJECT_RULES`, and a `worktree: false` session, which keeps the workspace's current branch unchecked. Sessions expose `spur-branch create|rename <name>` and block `git push` on a non-matching branch.

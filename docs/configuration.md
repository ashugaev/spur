...
Spur ToDo nudge events: `session.todo.nudge_failed` (one nudge attempt failed; retried on a doubling backoff, 2 minutes to a 30-minute cap; emitted for every transient failure), `session.todo.nudge_disabled` (nudges stopped for this session; `kind` `ledger_corrupt` — the ToDo ledger replays to an invalid transition and cannot succeed — or `target_gone` — the tmux target is missing). `session.todo.nudge_disabled` is emitted at most once per session per liveness episode: a session that goes terminal and is later reopened starts a new episode and may emit it again. The give-up itself is in-memory: a daemon restart re-probes once, and a `target_gone` give-up also re-probes after the session's pane is relaunched (`spur restore` or a wake). A repaired ledger resumes nudges after a daemon restart. The session record carries the last give-up as `todoNudgeDisabled` (`kind`, `reason`, `atMs`); it records the last observed give-up, not a live gate, and is cleared by the next successful nudge.

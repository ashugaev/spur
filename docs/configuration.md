...
- `projects.<id>.branchNaming.regex`: optional JavaScript regex. Validates explicit and trigger branches; validates preflight and fallback branches inside `resolveSpawnBranch` unless `skipBranchNamingValidation` (set when preflight answered `NO_PROJECT_RULES`). A `worktree: true` spawn with no branch uses the session ID as fallback — a regex that excludes names like `spur-c527` rejects that spawn. When a non-explicit chosen branch is already checked out in another worktree, Spur falls back to the session ID and still validates that name (`skipBranchNamingValidation` does not apply on that path). `worktree: false` keeps the workspace's current branch unchecked unless an explicit branch override is present. Sessions expose `spur-branch create|rename <name>` and block `git push` on a non-matching branch.

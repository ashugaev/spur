...
- `SPUR_WEB_URL`: optional process environment value, base URL of this instance's web UI. Sources that call the web UI (Telegram voice transcription, see [voice.md](voice.md#telegram-voice-notes)) use it instead of `http://127.0.0.1:<ui.port>`. Set to a non-loopback URL when `WEB_HOST` binds off loopback; set to an empty string to disable the feature for this instance (isolated daemons always set it empty, so they never reach the host's web UI). Unset falls back to `http://127.0.0.1:<ui.port>`.

...
A session bound to a PR number GitHub reports as nonexistent stops signal polling for that PR number after one attempt and logs `source.poll.disabled` once. The disable persists per source under `<dataDir>/source-state/github-poll-disabled/<projectId>/<sourceId>.json`, sticky across source reload and daemon restart. Daemon or source reload no longer re-probes a missing PR (unlike the prior in-memory-only disable); use [`spur source poll-enable`](commands.md#source) or rebind after fixing auth or the PR binding. It clears on rebinding away from that PR number (including rebinding back after an intermediate rebind) or via that command — which clears both the durable registry entry and any live in-process gate on the running source, so polling resumes on the next tick with no daemon restart or config reload needed, even when an earlier registry write for that session had failed to persist. `source.poll.disabled` on a live PR usually means the token lost repo visibility; fix auth and rebind to a different PR number, or run `spur source poll-enable` once auth is fixed. Any other poll failure retries on a doubling backoff (2 minutes to a 30-minute cap) instead of every cycle.

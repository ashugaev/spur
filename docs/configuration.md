...
Spur ToDo defaults off. Set instance `todo.enabled: true` to enable unoverridden projects. `projects.<id>.todo.enabled` takes precedence for new sessions. Each session pins the resolved value at creation; config reloads and worktree edits do not change it. Disabled sessions expose no ledger, tools, prompt, gate, nudge, completion event, or dashboard/detail state. Existing ledger bytes stay untouched. Command and completion behavior: [todo](commands.md#todo).
...
- `todo.enabled`: optional boolean, default `false`. Instance fallback for new sessions.
- `projects.<id>.todo.enabled`: optional boolean. Overrides instance `todo.enabled` for new sessions in that registered project. Existing sessions retain their pinned value. See [ToDo](commands.md#todo).

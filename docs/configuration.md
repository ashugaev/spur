...
Agent-initiated sends go the other way: [source reply](commands.md#source-reply) reaches the session's latest inbound chat, or `chatId` when it has none. That first send claims the chat for the session unless another session already holds it, so the user's typed reply routes back — including after `/unwatch`, which detaches until the next agent send. It also enrols the session in the attention pushes below, which before were reserved for `/watch` and `autoSpawn` sessions. Pending button choices are capped at 200 per source, evicted whole offers oldest-first, and die with the session. The launch prompt advertises Telegram only when that source has `chatId` and a `telegram:message` send trigger; either one missing keeps it silent.

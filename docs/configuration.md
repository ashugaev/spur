...
Agent-initiated sends go the other way: [source reply](commands.md#source-reply) reaches the session's latest inbound chat, or `chatId` when it has none. That first send claims the chat for the session unless another session already holds it, so the user's typed reply routes back. Pending button choices are capped at 200 per source, evicted whole offers oldest-first, and die with the session. Inbound delivery needs a `telegram:message` send trigger on that source — without one, and without `chatId`, the launch prompt stays silent about Telegram.

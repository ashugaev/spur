...
- `projects.<id>.sidecars.<name>.ports.<id>`: optional map. The `<id>` key follows the same id pattern as other Spur ids. One entry reserves one host port.
- `projects.<id>.sidecars.<name>.ports.<id>.env`: required string. The environment variable the reserved port is published under IN THE SIDECAR's OWN PROCESS — never exported into the agent session. Read it from inside a session with `"$SPUR_SESSION_TOOL_DIR/spur-sidecar" ports`, see [Sidecars](commands.md#sidecars). Not validated as a shell identifier and not checked for uniqueness across sidecars — two sidecars can declare the same `env` name.
...
- `projects.<id>.sidecars.<name>.ports.<id>.url`: optional. At most one port per sidecar may set it; it must carry no explicit port, path, query, or fragment, and the sidecar name must match the slot-label pattern. `{port}` is substituted with the reserved port to build the dashboard link.

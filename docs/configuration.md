...
- `admission.enabled`: optional boolean, default `true`. `false` disables cap and enforced memory-guard refusals for spawn and restore; the memory guard still evaluates and logs report-only. Instance config only — a project-config `admission` block parses without error and is discarded.
...
A configurable cap on concurrent live sessions, enforced at spawn and at restore. One live-session slot covers the agent and its MCP/service sidecar processes. Over cap, the caller gets a `429` whose message names the cap, total slots claimed split into live and in-flight reservations, and up to 3 candidate session ids to stop (stalest `updatedAt` first). Per-project denials list sessions from that project; global denials list the fleet. When every slot belongs to an in-flight spawn, the message says to wait for it to finish before stopping a session or retrying. The cap never acts retroactively — sessions already live above a lowered cap keep running; nothing is killed, paused, or reconciled down to fit.

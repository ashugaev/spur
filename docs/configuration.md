...
- `SPUR_WEB_URL`: optional process environment value, base URL of this instance's web UI; trailing slashes stripped. Sources that call the web UI (Telegram voice transcription, see [voice.md](voice.md#telegram-voice-notes)) use it in place of the `http://127.0.0.1:<ui.port>` default. Non-loopback URL for a `WEB_HOST` bound off loopback. Empty string disables the feature for this instance; isolated daemons set it empty.

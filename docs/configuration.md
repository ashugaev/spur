...
Agent process events, all from the pre-kill/post-kill pane check that runs before a teardown or relaunch: `session.agent_process.pane_pid_unreadable` (warn; the tmux pane's pid could not be looked up, so the prior agent process's exit can't be confirmed; `sessionId` only, no `details`). `session.agent_process.capture_unavailable` (warn, or error when the kill is guarding a relaunch; the process table could not be read before killing the pane, so "no survivors" can't be verified; a relaunch-guarding kill then refuses to launch a replacement; `sessionId` only, no `details`). `session.agent_process.capture_blind` (error; a relaunch was refused — process capture came back empty while the pane's occupant still read alive; `details.tmuxSession`, `details.agent`). `session.agent_process.survivors` (error; one or more captured agent processes were still alive after `SIGKILL`; `details.pids`). `capture_blind` and `session.runtime.pane_child_fallback` both fire when the launch command's derived binary doesn't match the agent's canonical process name — not only for an agent executable override (`SPUR_*_BIN`).

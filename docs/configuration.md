...
- `send.interrupt`: optional boolean, default `false`. `false` queues while `working`/`needs_input`, dedupes, flushes when `waiting` after the send window expires (default 30 s, controlled by `SPUR_IDLE_WAIT_BEFORE_FLUSH_MS`). `true` interrupts immediately while `working`; `waiting` and `needs_input` sessions still wait out the send window, then deliver with `interrupt: false`. `cursor` sessions never receive the `Ctrl-C` keystroke — the message is typed into the running turn, which cursor-agent queues itself; `claude` and `codex` are interrupted first.

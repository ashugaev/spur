...
Chats and forum topics bind to sessions with `/watch`. Without an id, Spur replies with an inline picker: sessions from all connected projects grouped by project, each labeled with its session title, plus a back button. `/watch <sessionId>` binds directly. A message from an allowed user in a chat with no live bound session auto-spawns an ephemeral session and binds the chat to it, one per chat; see `autoSpawn` below. One bot token serves all projects; access stays controlled by that source's `allowedUsers` / `allowedChats`. `/unwatch` drops the chat's binding; a later agent send binds it again. `/watch@otherbot` is ignored in group chats. Bound messages, and messages that spawn a session, reach the agent with a contract: the requester sees only replies sent with `spur source reply "<message>"`, terminal output is invisible.

Agent-initiated sends go the other way: [source reply](commands.md#source-reply) reaches the session's latest inbound chat, or `chatId` when it has none. That first send claims the chat for the session unless another session already holds it, so the user's typed reply routes back. An agent-initiated send also enrols the session in the attention pushes below. Pending button choices are capped at 200 per source, evicted whole offers oldest-first, and die with the session. The launch prompt advertises Telegram only when that source has `chatId` and a `telegram:message` send trigger; either one missing keeps it silent.

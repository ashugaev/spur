...
`http.request.failed` carries `status`, the HTTP status sent for that request, on two paths only: the missing-resource handler and the terminal catch. Other failure paths — malformed request, sidecar port conflict, todo ledger corruption — omit it. On the routes that document a `404` for an unknown `:id` ([daemon-api.md](daemon-api.md#daemon-http-api)), that failure logs `level: "warn"` with `status: 404`; the terminal catch logs `level: "error"` with `status: 500`.

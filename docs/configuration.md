...
Chats and forum topics bind to sessions with `/watch`. Without an id, Spur replies with an inline picker that lists sessions from all connected projects, grouped by project; selecting a project shows its sessions, labeled with the session title when it has one, and a back button returns to the project list. `/watch <sessionId>` binds directly. Cross-project binding is intentional: a single bot token serves all projects, and access remains controlled by that source's `allowedUsers` / `allowedChats`. `/watch@otherbot` is ignored in group chats. Bound messages, and messages that spawn a session via Telegram, reach the agent with a contract that states the requester only sees replies sent with `spur source reply "<message>"` and that terminal output is invisible.

Bound chats get proactive pushes from the attention monitor: `needs_input`, `error`, and `rate_limited` each push once on entry (with a pane tail for the first two), and the forum topic name tracks state. Notice text and topic name carry the session title when the session has one. A `working`→`waiting` transition with no reply since the last inbound message nudges the chat once. `complete`/`kill` send a farewell and close the topic. Every send is best-effort — a Telegram failure never blocks the monitor tick or cleanup.

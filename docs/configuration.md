...
Registry scans canonicalize registered paths, cache each merge result by file `stat` (size/mtime), and invalidate a path's cache entry on content or removal change. A path only leaves the registry through the boot/connect-time prune (see [Config registry](#config-registry)); a scan-time lookup or parse error instead leaves it registered and retries on the next scan. The instance config is never removed. A canonical problem path emits at most one `daemon.registry.warning` per daemon lifetime.

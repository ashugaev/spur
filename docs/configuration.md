...
Every `http.request.failed` carries `status`, the HTTP status sent for that request. `level` follows the status: `5xx` logs `error`, anything below logs `warn`. So an unknown `:id` on the routes that document a `404` ([daemon-api.md](daemon-api.md#daemon-http-api)) logs `level: "warn"` with `status: 404`, and an unhandled error logs `level: "error"` with `status: 500`.

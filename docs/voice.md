...
A voice note in a [Telegram source](configuration.md#events) chat becomes text. Chat bound with `/watch`: transcript goes to the agent as typed text. After `/spawn <agent>` and a project pick, no binding yet: transcript becomes the spawn prompt. A voice note can never satisfy the pending project pick itself — only a project-keyboard tap does; while a pick is still pending, the transcript is treated as any other free text (see [Telegram binding](configuration.md#telegram-binding)). Transcript is never parsed as a command, even one starting with `/`.

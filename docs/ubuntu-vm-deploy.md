...
- `Environment=PATH=...` — prepends the nvm bin dir when node is under `~/.nvm/versions/node/*/bin/node`, then user-scoped agent CLIs
- `KillMode=process` — restart kills only the daemon node process; tmux sessions survive
- starts with `{{NODE_BIN}} {{SPUR_ROOT}}/v2/dist/cli.js daemon start` (resolved at deploy time)
...
- `ExecStart={{PNPM_BIN}} ui:start` (resolved at deploy time)

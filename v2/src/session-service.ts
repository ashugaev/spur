...
function buildInitialMessage(
  ...
  let base = withSessionArtifactInstructions(
...
export class SessionService {
  ...
  async spawn(request: SpawnSessionRequest): Promise<SessionView> {
    ...
    try {
      ...
      stage = "tools.setup";
      const sessionToolDir = this.prepareSessionTools(sessionId, agent, project, request.project);
  ...
  private async prepareBackgroundSpawn(request: SpawnSessionRequest): Promise<PreparedSpawn> {
    ...
    try {
      ...
      return {
        ...
        placeholder,
        sessionToolDir: this.prepareSessionTools(sessionId, agent, project, request.project),

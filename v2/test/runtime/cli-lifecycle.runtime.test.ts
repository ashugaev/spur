...
`,
  ...
  it("updates live session status through SPUR_STATUS_COMMAND and clears it on send", async () => {
    const port = await findFreePort();
    const context = await createRuntimeTestContext(port);
    ...
    const helperPath = join(context.dataDir, "bin", "spur-status");
    expect(existsSync(helperPath)).toBe(true);

    const helperEnv = {
      ...context.env,
      SPUR_DATA_DIR: context.dataDir,
      SPUR_PROJECT: spawned.project,
      SPUR_SESSION: spawned.id,
    };

    await execFileAsync(helperPath, ["needs_input"], { env: helperEnv });
    const waitingInput = await pollUntil(
      async () =>
        JSON.parse(
          (await context.execCli(["--config", configPath, "list", "--json"])).stdout,
        ) as SessionView[],
    ...
    expect(waitingInput[0]?.state).toBe("needs_input");

    await execFileAsync(helperPath, ["done"], { env: helperEnv });
    const done = await pollUntil(
      ...
      {
        timeoutMs: 15_000,
        accept: (value) => value[0]?.status === "done" && value[0]?.state === "done",
      },
    );
    ...
    await expect(execFileAsync(helperPath, ["bogus"], { env: helperEnv })).rejects.toMatchObject({

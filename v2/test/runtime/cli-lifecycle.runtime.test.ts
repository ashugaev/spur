...
async function readTmuxOption(sessionName: string, option: string): Promise<string> {
  const { stdout } = await execFileAsync("tmux", ["show-options", "-t", sessionName, "-v", option]);
  return stdout.trim();
}
...
describe.skipIf(!tmuxOk)("Spur CLI lifecycle (runtime)", () => {
  ...
  it("updates session context through hidden manual and agent-side commands", async () => {
    const port = await findFreePort();
    const context = await createRuntimeTestContext(port);
    const sessionPrefix = `rt-context-${port}`;
    activeContexts.push({ context, sessionPrefix });
    await syncTmuxEnvironment({
      PATH: context.env.PATH,
      SPUR_FAKE_AGENT_LOG_DIR: context.agentLogDir,
    ...
    const configPath = await context.writeConfig("context.yaml", baseConfig(context, sessionPrefix));
    const daemon = await context.startDaemon(configPath);
    currentActiveContext().daemonPid = daemon.info.pid;
    ...
    await context.execCli([
      ...
      "send",
      spawned.id,
      "run-shell:spur internal context set-link tracker https://tracker.test/T-42 --label T-42",
      "--json",
    ]);
    const trackerLog = await pollUntil(() => context.readAgentLog(spawned.id), {
      timeoutMs: 15_000,
      accept: (value) =>
        value.includes(
          "shell ok: spur internal context set-link tracker https://tracker.test/T-42 --label T-42",
        ) ||
        value.includes(
          "shell failed: spur internal context set-link tracker https://tracker.test/T-42 --label T-42",
        ),
    });
    expect(trackerLog).toContain(
      "shell ok: spur internal context set-link tracker https://tracker.test/T-42 --label T-42",
    );
    ...
    await context.execCli([
      ...
      "send",
      spawned.id,
      "run-shell:spur internal context set-link pr https://example.test/pr/42 --label PR-42",
      "--json",
    ]);
    const prLog = await pollUntil(() => context.readAgentLog(spawned.id), {
      timeoutMs: 15_000,
      accept: (value) =>
        value.includes("shell ok: spur internal context set-link pr https://example.test/pr/42 --label PR-42") ||
        value.includes(
          "shell failed: spur internal context set-link pr https://example.test/pr/42 --label PR-42",
        ),
    });
    expect(prLog).toContain(
      "shell ok: spur internal context set-link pr https://example.test/pr/42 --label PR-42",
    );
    ...
    const listed = await pollUntil(
        ...
        ) as SessionView[],
      {
        timeoutMs: 15_000,
        accept: (value) =>
          value[0]?.taskTitle === "Tighten review loop" &&
          value[0]?.links["tracker"]?.label === "T-42" &&
          value[0]?.links["pr"]?.label === "PR-42",
      },
    );

    const statusLeft = await pollUntil(() => readTmuxOption(spawned.id, "status-left"), {
      timeoutMs: 15_000,
      accept: (value) => value.includes("Tighten review loop"),
    });
    const statusRight = await pollUntil(() => readTmuxOption(spawned.id, "status-right"), {
      timeoutMs: 15_000,
      accept: (value) => value.includes("tracker T-42") && value.includes("pr PR-42"),
    });

    expect(listed[0]?.taskTitle).toBe("Tighten review loop");
    expect(listed[0]?.links["tracker"]).toEqual({
      url: "https://tracker.test/T-42",
      label: "T-42",
    });
    expect(listed[0]?.links["pr"]).toEqual({
      url: "https://example.test/pr/42",
      label: "PR-42",
    });
    expect(statusLeft).toContain("Tighten review loop");
    expect(statusRight).toContain("tracker T-42");
    expect(statusRight).toContain("pr PR-42");
  });

  it("rejects invalid hidden context commands through the built CLI", async () => {
    const port = await findFreePort();
    const context = await createRuntimeTestContext(port);
    const sessionPrefix = `rt-context-error-${port}`;
    activeContexts.push({ context, sessionPrefix });
    const configPath = await context.writeConfig(
      "context-error.yaml",
      baseConfig(context, sessionPrefix),
    ...
    await expect(
      ...
      stderr: expect.stringContaining("slot must match"),
    });
  });

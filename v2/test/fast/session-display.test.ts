...
describe("session-display", () => {
  ...
  it("keeps done sessions above active work but below errors", () => {
    const ordered = sortSessionsForList([
      session({ id: "api-3", state: "working" }),
      session({ id: "api-1", state: "done", status: "done" }),
      session({ id: "api-2", state: "error", status: "errored" }),
    ]).map((entry) => entry.id);

    expect(ordered).toEqual(["api-2", "api-1", "api-3"]);
  });

...
const appendStatusInstructionsMock = vi.fn();
const ensureStatusCommandMock = vi.fn();
...
vi.mock("../../src/agent-status.js", () => ({
  appendStatusInstructions: appendStatusInstructionsMock,
  ensureStatusCommand: ensureStatusCommandMock,
}));
...
describe("SessionService", () => {
  beforeEach(() => {
    ...
    appendStatusInstructionsMock
      .mockReset()
      .mockImplementation((prompt: string) => `${prompt}\nstatus-instructions`);
    ensureStatusCommandMock.mockReset().mockReturnValue("/tmp/spur-bin/spur-status");
  ...
  it("spawns a session through one clear path and returns the enriched view", async () => {
    ...
    expect(createTmuxSessionMock).toHaveBeenCalledWith({
      ...
      env: {
        ...
        SPUR_PROJECT: "api",
        SPUR_AGENT: "claude",
        SPUR_DATA_DIR: "/tmp/spur-data",
        SPUR_STATUS_COMMAND: "/tmp/spur-bin/spur-status",
  ...
  it.each([
    { status: "needs_input", state: "needs_input" },
    { status: "done", state: "done" },
  ] as const)(
    "preserves agent-written %s before the final spawn record write",
    async ({ status, state }) => {
      readSessionMock.mockReturnValueOnce({
        ...
        updatedAt: "2026-03-18T10:05:00.001Z",
        slots: {
          title: "Immediate title",
          links: [{ label: "tracker", url: "https://tracker.example.com/42" }],
        },
      });
      ...
      expect(writeSessionMock.mock.calls[1]?.[1]).toMatchObject({
        status,
        slots: {
          title: "Immediate title",
          links: [{ label: "tracker", url: "https://tracker.example.com/42" }],
  ...
  it.each(["needs_input", "done"] as const)(
    "send clears explicit %s back to running",
    async (status) => {
      readSessionMock.mockReturnValue({
        id: "api-1",
        project: "api",

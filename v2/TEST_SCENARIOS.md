...
- `POST /sessions/:id/opened` persists `lastOpenedAt`, and session views expose `hasUnseenAttention` only when the current state is `waiting` or `needs_input` after the last open.

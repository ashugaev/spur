...
- Cursor's native tool-permission/allowlist gate (`Not in allowlist: ...`) renders only in the tmux pane, never in the agent transcript jsonl, so `classifyCursorJsonlState` alone reports `working`; `scanTmuxCursorPermissionPrompt` scans the pane for the gate's anchors and overrides a `working` or `waiting` Cursor session to `needs_input`, while a live `rate_limited` classification still takes priority over the pane scan.

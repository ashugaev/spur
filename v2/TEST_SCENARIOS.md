...
- Claude sessions always get a Claude Stop-hook `settings.json` (`auto-push.sh claude` close-out check) during spawn, pipeline-spawn, recovery, and restore regardless of `restrictWrites`, while the `PreToolUse` write-deny block in that same settings.json stays conditional on `restrictWrites`.

...
- Session state subscriptions live on the subscriber session record, validate subscriber and target sessions plus canonical states, stay hidden from root help, expose internal daemon routes, and deliver one queued message through the normal send path on matching real state transitions.

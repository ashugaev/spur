...
- Claude sessions skip hook state scripts (`spur-agent-state-updater.mjs`, `spur-agent-state`) during spawn and recovery.
- Claude sessions get a Claude `settings.json` on spawn, pipeline-spawn, recovery, and restore with a guarded Stop-hook close-out check (`[ -x .claude/hooks/auto-push.sh ] && .claude/hooks/auto-push.sh claude || exit 0`, a no-op on worktrees that don't ship that script) when `restrictWrites` is not set, or a `PreToolUse` write-deny block instead when `restrictWrites` is set; the two hooks are mutually exclusive so restrict-writes sessions are never forced into the git close-out gate.

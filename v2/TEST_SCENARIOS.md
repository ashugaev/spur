...
- Root help also exposes `doctor`, and `doctor --help` explains the read-only default, the `--scaffold` write flow, and the follow-up auto-connect flow through `list` or `spawn`.
- Root help exposes `doctor`, `spawn`, `shepherd`, `list`, `send`, `pause`, `complete`, `kill`, `respawn`, `session-memory`, and `service`, keeps the branded help output, and hides the internal `daemon`, `slots`, `subscribe`, and `sidecar` commands.
...
- `doctor` probes the daemon's `/info` endpoint (never the heavier `GET /sessions` view) for liveness and reuses that same response for the version-drift check; when systemd already reports the unit active but the probe fails, reports a `*-reachable` failure as `error` for a definitive failure (`connection-refused`/`http-error`/`unknown`) but as `warn` for a bare `timeout` (may be under load, never exit-code-affecting on its own); reports a `*-port-conflict` as `error` only when the busy port's listener PID does not match the unit's own systemd `MainPID` (never a second `/info` round trip); and reports the plain not-yet-started state as `warn` only when systemd itself has not already reported the same inactive fact as `error` (single owner, never duplicated at two severities).
- On an initialized host (systemd units installed), `doctor` reports `spur-daemon`/`spur-web` "not active" as `severity:error` (static, regardless of `ok`) so a dead daemon is exit-code-affecting even after a deliberate `systemctl stop`; on a never-initialized host (units not installed), those checks are never emitted and `doctor` still exits 0 on `systemd-units`/`linger`'s `warn`-only failures.
...
- `doctor` reports `daemon-dist-integrity`/`web-dist-integrity` errors when an installed unit's `ExecStart` `.js` target file is missing, and pushes no `web-dist-integrity` check at all when `ExecStart` has no resolvable `.js` target (system-scope `pnpm ui:start`).
- `doctor` runs `node -e "require('node-pty')"` out-of-process against the web unit's resolved bundle directory and reports `web-native-module-load` error on failure, but pushes no check at all when neither candidate bundle directory has `node-pty` installed.
- `doctor` enriches the `spur-daemon`/`spur-web` detail with the unit's `ActiveState`/`SubState`/`NRestarts`/`ExecMainStatus` when systemd reports it inactive, naming a crash-loop instead of suggesting a bare restart, and keeps today's plain "not active" wording for a clean stop.
- `doctor` write-probes `worktreeDir` and `dataDir` and reports an error when unwritable, checks `dataDir`'s free disk space/inodes via `df` and reports an error only near-zero (skipping as `info` when `df` is unavailable or non-numeric), and skips all three checks entirely on a never-initialized host (no readable instance config) or before the daemon has ever created those directories (systemd units not installed).
- `doctor` probes the daemon at its configured `server.host` (not a hardcoded loopback) so a daemon correctly bound to a non-loopback address is not falsely reported unreachable.
- `doctor` reports `web-ui-port-drift` as `warn` (never exit-code-affecting) when the configured `ui.port` differs from the web unit's actual listen port, once both the instance config and the web unit are installed.
- `loadInstanceConfigReadOnly` never bootstrap-writes the pinned instance config file, distinguishing an absent file from a real parse failure; `doctor` surfaces the latter as an `instance-config-corrupt` error without throwing, while `resolveDaemonPortReadOnly`'s own best-effort fallback keeps working from the same corrupt file in the same run, and no other instance-config-derived check cascades a second error from it.
- `checkProjectWorkspace` validates a configured project's `path` existence, then (only in worktree mode) whether it is a git repository, then whether `defaultBranch` resolves locally or at `origin`, skipping later checks once an earlier one in the chain fails or worktree mode is off, and accepting a remote-only or local-only branch as resolvable.
- `doctor`'s `node-version` check actually reads `engines.node` from this package's own installed `package.json` (mirroring `version.ts`'s resolution) and reports a real pass/fail, rather than silently degrading to a permanent "skipped" no-op.
...
- Telegram source command handling keeps `/start`, `/help`, `/agents`, `/watch`, `/spawn`, and `/unwatch` inside Telegram, rejects unauthorized users and callbacks before side effects, and can spawn plus bind a new allowed agent session.
- Telegram source replies edit the pending Telegram status message when one exists; otherwise group replies can lazily create a per-agent forum topic with a status emoji title, persist the topic binding, and send the reply there with main-chat fallback when topic creation is unavailable.
- Telegram source resolves its own bot username via `getMe` at start, matches only bare or own-addressed commands, stays silent on foreign/unknown slash text in groups, unbinds and reports dead sessions before emitting, wraps spawn prompts with the `spur source reply` contract, surfaces spawn and `/watch` bind failures with redacted tokens, and logs a `source.telegram.auth_failed` event plus a distinct 409 polling-conflict message on runner errors.
- Telegram source pushes attention (needs-input/error/rate-limited) notices with a tmux pane tail to bound chats without firing on the baseline pass, nudges once on a working-to-waiting transition with no reply since the last inbound, sends a farewell and closes the forum topic before unbinding on completion or kill, and keeps every send best-effort so a Telegram failure never breaks the monitor or cleanup.
...
- Session state subscriptions persist on subscriber session records, validate subscriber and target sessions plus canonical states, stay hidden from root help, expose internal daemon routes, and deliver one queued message through normal send on matching state transitions.
...
- Rate-limit detection (`rate-limit-detect.ts`) reads structured agent sources first and overrides a running session's state to `rate_limited`: Codex via the latest rollout `token_count.rate_limits` (`rate_limit_reached_type`, `credits.has_credits === false`, or a window at 100% used), Claude via a trailing synthetic `error: "rate_limit"` transcript record, Cursor via usage-limit marker text; when the structured source carries no usable data it falls back to scanning the tmux pane buffer for rendered markers, never starting from tmux.
- When a Claude session's tmux pane shows Claude Code's interactive `stop and wait for limit to reset` / `ask your admin for more usage` usage-limit menu, `classifySessionRecord` sends a bare Enter keystroke via `sendSubmitKeyToTmux` to confirm the pre-selected `stop and wait` option (gated by a 10s per-session cooldown so a redraw-lag doesn't double-send), logging `session.rate_limit.usage_menu_confirmed` on success or `session.rate_limit.usage_menu_confirm_failed` on failure; this is scoped strictly to Claude sessions and never fires for Codex or Cursor.
- The default (queuing) `send()` path is unaffected by a session's live `rate_limited` state and keeps queuing normally, delivering once the state leaves `rate_limited` and becomes `waiting`; only the immediate-delivery paths — `deliver()` and `send({ queue: false })` — throw `SessionRateLimitedError` and log `session.message.suppressed_rate_limited` instead of sending, restart-safe via a fallback to the persisted `rateLimitedAt` marker when no live classification exists yet, and trigger-sourced deliveries suppressed this way leave their pending batch intact for a later retry instead of being dropped.
...
- `POST /sessions/:id/opened` persists `lastOpenedAt`, and session views expose `hasUnseenAttention` only when the current state is `needs_input` and the session's last activity is newer than `lastOpenedAt` (no in-memory state-history dependency, so a daemon restart with no recorded `lastOpenedAt` renders bright, not dimmed).
...
- `doctor` reports a configured project whose `path` does not exist as a distinct `project-path-exists:<id>` error with a non-zero exit, without throwing.

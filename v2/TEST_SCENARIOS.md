...
- Session-memory metadata helpers store one file per entry under `dataDir/session-memory/<sessionId>`, round-trip write and read, return null for a missing key, list an empty session as `[]` sorted by key, and delete the whole session directory.
- Session-memory HTTP routes expose `GET /memory`, `GET /memory/:key`, `PUT /memory/:key`, and `POST /memory/:key/resolve`, apply `kind=note`/`status=open`/`tags=[]` defaults on first `set`, preserve `createdAt` on a second `set`, mark entries `resolved` with `resolvedAt`, return 404 for a missing session or key, and 400 for an invalid key.
- `spur session-memory` exposes `list`, `get`, `set`, and `resolve` subcommands, each taking an explicit `<sessionId>`, with `set` accepting `--kind`, `--tags`, and `--json`.

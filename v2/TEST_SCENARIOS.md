...
- `send()` and `deliver()` drop outbound messages (including attachments-only sends) while a session's live state is `rate_limited`, logging `session.message.suppressed_rate_limited` without throwing, except the internal rate-limit-reactivation wakeup, which still delivers, and normal delivery resumes once the state leaves `rate_limited`.

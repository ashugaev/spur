...
- Claude server-error reactivation: a transcript record carrying `error: "server_error"` (with or without `apiErrorStatus`) as the last meaningful record — trailing `system`/`file-history-snapshot`/`progress` records skipped — classifies a running Claude session as `error` (never `rate_limited`, which keeps priority), persists `serverErrorAt`, clears it once a newer meaningful record appears, and re-sends `Claude hit a temporary server error earlier. Please try to continue the task now.` via `send({ queue: false })` (typed into the pane, not queued) at most once every 30 minutes while the live state stays `error`.
- A running claude session whose live state is `error` (the server-error wedge above) is treated as still active, not closed, everywhere `error` used to mean a dead session: `triggers.ts` keeps a pending send-batch queued instead of dropping it as `closed_session` and suppresses a duplicate work-item spawn exactly as it would for an active `working`/`waiting`/`needs_input` owner, while an ordinary queued `send()` (`tryDeliverQueuedMessage`) delivers immediately — typing the message itself is enough to un-wedge Claude — instead of sitting queued for up to 30 minutes waiting on the reactivation nudge. A session whose `status` is not `running` (a genuinely closed/errored session) is unaffected and still drops as `closed_session`.

...
- Claude JSONL fixture classification covers waiting reasons from real history (end_turn, stop_sequence, system, stop_hook_summary, file-history-snapshot, stale spur-0190 tail), all working sources (progress, fresh user message, fresh user tool_result, assistant streaming, fresh tool_use), and needs_input (stale tool_use).

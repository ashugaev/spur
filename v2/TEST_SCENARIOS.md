...
- `spawn --model <id>` threads a per-session model into the spawn request only alongside `--agent`; `resolveSpawnModel` lets an explicit request model win regardless of resolved agent, and otherwise applies the resolved agent's normalized `agentDefaults.model` without bleeding onto another agent.
- Config normalizes canonical `agentDefaults` shorthand/object entries and legacy `defaultModels`/`defaultEfforts` aliases into one internal map; aliases fill missing fields, equal duplicates pass, differing duplicates fail by field, and invalid agents, properties, entries, empty strings/objects, and Codex effort fail at the parse boundary.
- `resolveSpawnEffort` mirrors `resolveSpawnModel`'s precedence chain for reasoning effort: an explicit request effort wins, then the resolved agent's normalized `agentDefaults.effort`, then a Claude-only code-level default; Claude threads the resolved effort through a native `--effort <value>` launch flag, while Cursor appends an `[effort=...]` bracket suffix only when a concrete (non-auto) model is set and has no code-level default of its own.
- Config parses trigger spawn block `model` and `effort`, requires a matching `agent`, and rejects Codex effort.
- `resolveSpawnEffort` throws at the spawn boundary when an explicit request effort targets `codex` (mirroring the config-parse-time rejection) and stays silent when `codex` has no requested effort; `respawn`/`handoff` carry a source session's effort into the replacement session only when the request supplies no explicit effort and the resolved agent matches the source session's agent, otherwise the carried effort is dropped without throwing.

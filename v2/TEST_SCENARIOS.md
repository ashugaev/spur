...
- Claude JSONL classifier: `classifyClaudeJsonlState` maps assistant+stop_reason→waiting, assistant+tool_use within 15s→working, assistant+tool_use stale→needs_input, system/stop_hook_summary/file-history-snapshot→waiting, user→working, progress→working, empty→working.
...
- Claude agent status detection: spawn produces `waiting` (end_turn JSONL), `send` produces `working` (user JSONL), `show-waiting-menu` produces `needs_input` after the 15s stale window, `slow-tool-result` stays `working` until the delayed tool result lands, and normal message exchange cycles waiting→working→waiting.

...
- Claude pane question fallback: when a live Claude session has JSONL `waiting` or no JSONL yet, a captured pane showing the Claude question chooser is promoted to `needs_input`; JSONL `working` and JSONL `needs_input` stay authoritative, and Codex/Cursor state paths do not read this fallback.

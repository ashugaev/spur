...
- Agent status helper keeps one path: injected `SPUR_STATUS_COMMAND` updates live session metadata to `running`, `needs_input`, or `done`, and `send` clears explicit `needs_input` or `done` back to `running`.
- Session state classification collapses public session status to `working`, `waiting`, `needs_input`, `done`, `stopped`, `error`, and `killed`, including plan-mode menus, permission prompts, Codex trailing UI, and agent-managed status overrides.
...
- The injected `SPUR_STATUS_COMMAND` helper updates a live session to `needs_input` or `done`, and `send` resets it to `running` without restarting the session.
...
- `SPUR_STATUS_COMMAND` rejects unsupported status values.

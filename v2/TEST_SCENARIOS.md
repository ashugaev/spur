...
- `resolveSpawnEffort` mirrors `resolveSpawnModel`'s precedence chain for reasoning effort: an explicit request effort wins, then the project `defaultEfforts` entry keyed by the resolved agent, then a Claude-only code-level default; Claude threads the resolved effort through a native `--effort <value>` launch flag, while Cursor appends an `[effort=...]` bracket suffix only when a concrete (non-auto) model is set and has no code-level default of its own.
- Config parses a trigger spawn block `effort` and a project `defaultEfforts` map keyed by agent, and rejects `.effort` without `.agent`, an unknown `defaultEfforts` agent key, and a non-string `defaultEfforts` value at the parse boundary.

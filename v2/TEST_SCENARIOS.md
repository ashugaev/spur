...
- `GET /sessions/:id/core` and `SessionService.getCore` return a cheap first-paint view (record plus `state` from status fallback, `runtimeAlive`, `workspaceExists`, and `lastActivityAt`=`updatedAt`) without the heavy `services`/`sidecars`/`artifacts`/`deskGroupMembers`/`stateHistory` fields, probe tmux exactly once for non-terminal sessions and never for terminal ones, skip `classifySessionRecord`/`readRuntimeSnapshot`/`buildDeskGroupMembers`, and 404 on an unknown id like `GET /sessions/:id`.

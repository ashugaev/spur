...
Each live session also gets `SPUR_STATUS_COMMAND` in the shell environment.
Use it inside the session when the agent itself should move the session into a human-facing state:

```bash
"$SPUR_STATUS_COMMAND" needs_input
"$SPUR_STATUS_COMMAND" done
"$SPUR_STATUS_COMMAND" running
```
